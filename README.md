...
- **1-5등급 컷 계산**: 지필평가 교과목별 일람표를 넣으면 2022 개정 5등급 누적비율 기준 컷 점수 자동 산출
...
| (선택) 지필평가 교과목별 일람표 | .xlsx | 1~5등급 컷 계산에 사용. 반번호 매트릭스형 점수표와 환산점수/총점/원점수 열을 자동 인식 |
...
- 그래프의 성취도선, 9등급선, 5등급선, 학생 위치, 학생 이름은 필요에 따라 켜고 끌 수 있습니다.
- `포트폴리오 저장`을 누르면 현재 과목의 학생별 성취도, 9등급, 5등급, 환산점수를 앱 자료 폴더에 저장합니다.
- `학생 포트폴리오` 탭에서 `학생 선택`을 고르면 저장된 여러 과목 결과가 바로 정리되고, `상담 리포트 보기`로 한 학생의 포트폴리오를 딸깍 열 수 있습니다.
- 저장된 기록은 앱 자료 폴더에서 자동으로 불러오며, `저장자료 불러오기`로 새로 갱신할 수 있습니다.

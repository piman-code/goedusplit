...
Only the structure is used: item text never leaves this PC. HWPX and PDF are read
with built-in code; HWP (binary) needs the local ``kordoc`` CLI, which is run with
outbound network blocked. The teacher confirms the result before it is used.
"""
...
_SERDAP_HEADER = re.compile(  # "[서답형 1]", "서술형 2.", but not a range such as "서답형 1~2번은"
    r"^\s*[\[\(<【〈《]?\s*(서답형|서술형|논술형|단답형)\s*[-–]?\s*(\d{1,2})(?!\d)(?!\s*[~∼\-–]\s*\d)"
)
_SELECT_HEADER = re.compile(r"^\s*(\d{1,2})\s*([.)．])\s*(?=\S)")
_POINTS = re.compile(r"[\[\(〔［]\s*(\d{1,2}(?:\.\d{1,2})?)\s*점\s*[\]\)〕］]|배점\s*[:：]?\s*(\d{1,2}(?:\.\d{1,2})?)")
...
def _read_pdf(path: Path) -> str:
    try:
        ...
        raise ExamReadError("PDF에서 글자를 읽지 못했습니다. 스캔한 PDF는 지원하지 않습니다.") from exc


def _read_with_kordoc(path: Path, kordoc: str) -> str:
    env = dict(os.environ, KORDOC_OFFLINE="1")  # never download models or call out
    try:
        completed = subprocess.run(
            [kordoc, str(path), "--silent"], capture_output=True, text=True, encoding="utf-8",
            errors="replace", timeout=90, check=False, env=env,
        )
    ...
    if not text.strip():
        ...
        if not kordoc:
            raise ExamReadError(
                ...
                if suffix == ".hwp" else "파일에서 글자를 찾지 못했습니다."
            )
        text, how = _read_with_kordoc(path, kordoc), "kordoc(오프라인)"
    return text, how
...
def _points_in(text: str) -> list[float]:
    return [float(a or b) for a, b in _POINTS.findall(text)]
...
def parse_exam_structure(text: str) -> dict:
    ...
    headers = []  # (line index, type, number)
    ...
    for index, line in enumerate(lines):
        ...
        if serdap:
            kind, number = "서답형", int(serdap.group(2))
        else:
            select = _SELECT_HEADER.match(line)
            if not select:
                continue
            if select.group(2) == ")" and not _points_in(line):
                continue  # "2) 조건" inside an item; "3) ... [4점]" is still a header
            kind, number = "선택형", int(select.group(1))
        if expected[kind] <= number <= expected[kind] + _MAX_SKIP:
            headers.append((index, kind, number))
            expected[kind] = number + 1

    items = []
    for position, (start, kind, number) in enumerate(headers):
        ...
        choices = len({mark for mark in CHOICE_MARKS if mark in block})
